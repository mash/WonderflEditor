...
public class Editor extends TextEditUI {
	...
	protected var _livecoding:LiveCoding;
	...
	private function changeHandler(event:Event):void {
		...
		if (prevText != text) {
			if (preventFollowingTextInput) {
				...
				comparator.compare(prevText, text);//diff
				if (!ignoreChange) {
					entry = new HistoryEntry(comparator.commonPrefixLength);
			...
			if (LiveCoding.isLive) {
				if (entry) _livecoding.pushHistory(entry);
			}
		}
	}
	...
	override public function setSelection(beginIndex:int, endIndex:int):void 
	{
		...
		if (LiveCoding.isLive) {
			_livecoding.pushCurrentSelection(selectionBeginIndex, endIndex);
			//callLater(_livecoding.pushCurrentSelection, [beginIndex, endIndex], 1);
			callLater(function ():void {
				_livecoding.pushCurrentSelection(selectionBeginIndex, selectionEndIndex);
			});
		}
	}
