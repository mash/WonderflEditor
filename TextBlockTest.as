package
{
	...
	public class TextBlockTest extends Sprite
	{
		...
		public function TextBlockTest()
		{
			XML.prettyPrinting = false;
			stage.align = StageAlign.TOP_LEFT;
			stage.scaleMode = StageScaleMode.NO_SCALE;
			
			_block = new TextBlock(null);
			
			var tfm:TextFormat = new TextFormat('_typewriter', null, 0xffffff);
			...
			var rect:Rectangle = new Rectangle(0, 0, 0, 15);
			...
			var textElements:Vector.<ContentElement> = new Vector.<ContentElement>(len * 2 + 1, true);
			var j:int;	
			var l:int;
			var i:int;
			var tf:TextField;
			...
			while (i < len) {
				format_runs[i] = new Format(format_runs[i++]);
			}
			...
			trace("lap time : " + (getTimer() - t) + " ms");
			
			index = Math.min(pos, code_str.length);
			
			o = format_runs[0];
			j = -1;
			pos = 0;
			while (i < len) {
				str = code_str.substring(oo ? oo.end : 0, o.begin);
				elf = new ElementFormat(new FontDescription(tfm.font), tfm.size + 0, 0xffffff);
//				if (str.charAt(0) == "\r") str = str.substr(1);
				te = new TextElement(str, elf);
				textElements[pos++] = te;
				
				str = code_str.substring(o.begin, o.end);
				elf = new ElementFormat(
						new FontDescription(tfm.font, (o.bold ? FontWeight.BOLD : FontWeight.NORMAL), (o.italic ? FontPosture.ITALIC : FontPosture.NORMAL)),
						tfm.size + 0, o.color);
//				if (str.charAt(0) == "\r") str = str.substr(1);
				te = new TextElement(str, elf);
				textElements[pos++] = te;
				...
				if (j < 0 && o.begin > index) {
					j = pos;
					//break;
				}
			}
			
			trace(textElements.length, j);
			trace("lap time : " + (getTimer() - t) + " ms");
			
			group.setElements(textElements.slice(0, j));
			_block.content = group;
			
			l = 0;
			var textLine:TextLine = _block.createTextLine(null, 1000000, 0);
			while (l < lines && textLine) {
				textLine.x = 4;
				textLine.y = 15 * (l + 1) - 2;
				addChild(textLine);
				textLine = _block.createTextLine(textLine, 1000000, ++l);
			}
...
class Format {
	...
	public function Format(initObject:Object) {
		color = parseInt("0x" + initObject.color);
