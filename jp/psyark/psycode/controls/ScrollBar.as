...
public class ScrollBar extends UIControl {
	...
	private var handlePressX:Number = 0;
	private var handlePressY:Number = 0;
	...
	protected function trackMouseDownHandler(event:MouseEvent):void {
		var position:Number;
		if (direction == VERTICAL) {
			position = Math.max(0, Math.min(draggableSize, event.stageY - handlePressY));
		}
		moveCursor(event.stageY + handle.height / 2);
