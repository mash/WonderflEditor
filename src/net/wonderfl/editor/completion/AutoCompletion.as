...
public class AutoCompletion extends Sprite {
	...
	private function checkAddImports(name:String):void
	{
		...
		{
			trace("checkAddImports:not inScope");
			var missing:Vector.<String> = ctrl.getMissingImports(name, caret - name.length);
			if (missing)
			{
				trace("missing");
				//missing.forEach(trace);
				var sumChars:int = 0;
				for (var i:int = 0; i < missing.length; i++)
				{	
					var pos:int = fld.text.lastIndexOf('package', fld.selectionBeginIndex);
	...
	private function rePositionMenu():void
	{
		...
		background1.x = background2.x = list.x = rect.x + fld.lineNumWidth;
		background1.y = background2.y = list.y = rect.bottom + 2;
